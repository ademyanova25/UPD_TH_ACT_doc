...
def extract_text_from_pdf(pdf_bytes: bytes) -> TextExtractionResult:
    ...
    try:
        ...
        for image in images:
            page_text, used_variant = _ocr_with_auto_rotate(image, pytesseract)
        ...
        if text:
            diagnostics.append(
                "Текст извлечён OCR (tesseract) с автоповоротом/проверкой ориентаций: "
                + ", ".join(variants_used)
            )
        else:
            diagnostics.append(
                "OCR выполнен (включая автоповорот и проверку 0/90/180/270), но текст не найден."
            )
